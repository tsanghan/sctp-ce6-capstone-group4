...
data "aws_iam_policy_document" "external_dns" {
  count = local.enable_external_dns && length(var.external_dns_route53_zone_arns) > 0 ? 1 : 0
...
locals {
  ...
  perms         = concat(var.source_policy_documents, var.override_policy_documents, var.policy_statements, data.aws_iam_policy_document.external_dns)
}
